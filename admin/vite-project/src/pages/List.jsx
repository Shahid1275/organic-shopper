...
//       console.log('Received products from API at', new Date().toISOString(), ':', products);
//       setList(products);
...
//       await axios.delete(`${backendUrl}/api/product/remove/${productId}`, {
//         headers: {
//           Authorization: `Bearer ${token}`,
//         },
//       });
...
//       toast.success('Product removed successfully at ' + new Date().toISOString());
//     } catch (error) {
//       toast.error('Error removing product at ' + new Date().toISOString() + ': ' + (error.response?.data?.error || error.message));
//     }
//   };
...
//   // Function to get the price display for all sizes
//   const getPriceDisplay = (price, sizes) => {
//     console.log('Rendering price for product at', new Date().toISOString(), ':', { price, sizes });
...
//     const priceStrings = sizes.map(size => {
//       if (price[size] && price[size].display) {
//         return `${size}: ${price[size].display}`; // e.g., "S: $55.00"
//       }
//       const value = price[size]?.value || 0;
//       return `${size}: ${currency}${value.toFixed(2)}`;
//     });

//     return priceStrings.join(', '); // e.g., "S: $55.00, M: $55.00, L: $55.00, XL: $55.00"

//     // To display only one price (e.g., the first size), uncomment the following and comment out the above:
//     /*
//     const defaultSize = sizes[0]; // Use the first size
//     if (price[defaultSize] && price[defaultSize].display) {
//       return `${defaultSize}: ${price[defaultSize].display}`; // e.g., "S: $55.00"
//     }
//     const value = price[defaultSize]?.value || 0;
//     return `${defaultSize}: ${currency}${value.toFixed(2)}`;
//     */
//   };
