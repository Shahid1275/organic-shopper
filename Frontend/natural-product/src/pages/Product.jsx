...
const Product = () => {
  ...
  useEffect(() => {
    ...
    if (product) {
      setProductData(product);
      setMainImage(product.image[0] || '');
      setSelectedSize(product.sizes?.[0] || ''); // Pre-select the first available size
    }
  }, [productId, products]);
  ...
  // Get the price for the selected size, fallback to the first available price if no size is selected
  const currentPrice = selectedSize ? productData.price[selectedSize] : productData.price[productData.sizes[0]] || 0;
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex gap-2">
                {productData.sizes.map((size) => (
                  ...
                  >
                    {size}
                     {/* (${productData.price[size].toFixed(2)}) */}
                  </button>
                ))}
              </div>
            </div>
